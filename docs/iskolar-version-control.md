...
- **CI now enforces the gates.** `.github/workflows/ci.yml` runs on every push to `main` and every pull request: `npm ci` → `npm run lint` → `npm run typecheck` → `npm run test` → `npm run build`, plus a `gitleaks` secret scan (§5), an RLS integration job against a booted local Supabase stack (exercises `tests/integration/rls.test.ts`, no longer skipped), and a Playwright e2e smoke job. A red gate blocks the PR. The manual pre-push checklist in §7 is still the *first* line of defense (don't push red and wait for CI to tell you), but it is no longer the *only* one. Node is pinned to v22 in the workflow to match local dev.
...
**Before push:** 6. `git log` — does the commit sequence about to be pushed read cleanly? No "wip," "fix typo," "actually fix" chains that should've been squashed on a feature branch. 7. Run `npm run lint`, `npm run typecheck`, `npm run test`, and `npm run build` locally — all must pass. Don't push red. **CI (`.github/workflows/ci.yml`, §0) re-runs all four of these plus a gitleaks secret scan, the RLS integration suite against a booted local Supabase, and a Playwright e2e smoke on every push/PR — but running them locally first keeps the feedback loop fast and CI green.** If the change touches Supabase migrations, also run `npx supabase db reset` against a local stack (requires Docker Desktop) and confirm it applies with zero errors before pushing; the CI RLS job will boot its own stack and fail if a policy regressed. 8. Confirm target branch and remote (`git branch -vv`, `git remote -v`) — especially important once feature branches exist, to avoid pushing a feature branch's work directly onto `main`. 9. For anything schema/migration-related: has it been reviewed as a diff (`DEPLOYMENT.md` §5), not run ad hoc against a live database?
