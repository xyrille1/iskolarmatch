...
Migrations live in `supabase/migrations/`, applied in filename order (`20260101000001_...` → `20260101000014_...`). The later files each add one v2-backlog feature's tables/grants inline (no separate grant migration needed post-`...006`): `...007` eligibility-rule guidance, `...008` scholarship reports, `...009` push subscriptions, `...010` saved-list shares, `...011` saved profiles (FR20 digest), `...012` source-watcher (FR10), `...013` application tracker (FR21), `...014` source discovery (FR22). `npm run db:reset` runs `supabase db reset` against a local Supabase instance (`supabase/config.toml`, Postgres 17) for local development. Per `SECURITY.md` SEC-G5 and `docs/iskolar-version-control.md`, migrations are **forward-only** — a mistake gets a new migration, not an edited/rebased one, so the schema stays reconstructable from git history alone. Adding a table or policy without a migration file (e.g. via the Supabase dashboard) breaks that guarantee and must be avoided.
