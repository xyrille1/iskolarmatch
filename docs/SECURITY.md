...
### 3.6 Data access & RLS
Every table has RLS enabled; content tables have no write policy for any client-facing role (service-role only); user-owned tables (`saved_scholarships`, `reminders`, `push_subscriptions`, `saved_list_shares`, `saved_profiles`, `application_progress`, `requirement_checkoffs`) are strictly `auth.uid()`-scoped; `admin_users` allows only a self-read; `audit_log` is admin-read-only via a `SECURITY DEFINER` function, with no write policy for any role. Full policy list in `DATABASE.md` §5 — this doc intentionally doesn't duplicate it.
...
- **FR21 application tracker** (`lib/actions/application-tracker.ts`, tables `application_progress` / `requirement_checkoffs`) is an **authenticated-owner** write surface — **not** anon-write (contrast FR13's `scholarship_reports`) — using the session Supabase client with `auth.uid()`-scoped RLS, exactly like `reminders`/`saved_scholarships`. `user_id` always comes from the session, never a request param (§3.4). Inputs are Zod-validated: `status` against the DB CHECK enum, `notes` capped at 1000 chars (owner's own private text; React escapes on render), ids as UUIDs. It persists per-scholarship **tracking** state only, **not** the matching profile, so it does **not** touch SEC-G1 / the zero-persisted-profile posture — that remains FR20's sole, opt-in exception. Anonymous visitors keep the ephemeral (unpersisted) requirement checklist.
...
### 3.11 Source-discovery crawler posture (FR22)
The discovery crawler (`lib/source-discovery/`) fetches multiple pages per registered index page, so it is treated with the same SSRF posture as the source-watcher **and** adds good-citizen-crawler controls that the legality requirement rests on:
- **Same fetch guard, same allowlist.** It reuses `fetchSource` (`lib/source-watcher/fetch-source.ts`) unchanged — HTTPS-only, `isAllowlistedUrl()` gov.ph/edu.ph/curated allowlist, `isPrivateIp()` on every resolved IP, manual per-hop redirect revalidation, byte cap + timeout. In addition, both `source_index_pages.index_url` and `scholarship_candidates.detail_url` are allowlist-enforced at the DB layer by the `enforce_source_discovery_url_allowlist` trigger (migration `...014`), reusing the shared `is_allowlisted_url()` — so an off-allowlist page can never be registered or filed even via a direct service-role write. This is the same three-layer SEC-G3 discipline extended to the crawl surface.
- **robots.txt honored.** Before fetching an index page or any detail page, the crawler fetches and parses that host's `robots.txt` (`lib/source-discovery/robots.ts`, unit-tested) and skips disallowed paths. A missing/unreachable `robots.txt` is treated as unrestricted (the standard). Wildcard (`*`) and end-anchor (`$`) patterns and `Crawl-delay` are supported; longest-match-wins between Allow/Disallow.
- **Identifies itself + rate-limits.** Every request sends a self-identifying `User-Agent` with a contact URL (`DISCOVERY_USER_AGENT`), and a per-domain crawl delay (our floor, raised by any `Crawl-delay`, capped so a hostile directive can't stall the run) spaces successive fetches. A per-run detail-page ceiling bounds total load.
- **Facts, not prose; never auto-publishes.** The LLM is used only to (a) classify which extracted anchors are scholarship links — choosing from anchors we supply, so it can't fabricate a URL — and (b) extract draft facts from a public page. It never decides eligibility and never publishes. Candidates are written only to `scholarship_candidates` (RLS default-deny, service-role only); promotion (`lib/actions/discoveries.ts`) routes the **curator-reviewed** draft through the validated `upsertScholarship` action and creates a *draft* (unpublished) scholarship. Off-allowlist blogs/listicles are handled as human leads only and are never fetched or stored. Like the watcher, it touches no student profile data — no bearing on SEC-G1.
...
- [x] `tests/integration/rls.test.ts` — covers anon read-published/no-write/no-`allowlisted_domains`-read, plus explicit allow/deny cases for `source_index_pages`/`scholarship_candidates`/`scholarship_reports`/`push_subscriptions`/`saved_list_shares`/`saved_profiles`. Still self-skips locally without `TEST_SUPABASE_URL`/`TEST_SUPABASE_ANON_KEY` against a local Supabase stack, but the CI `rls` job (`.github/workflows/ci.yml`) boots that stack and sets those vars, so it runs (not skipped) on every push/PR.
- [ ] Two-user manual test that user A cannot read/write user B's `saved_scholarships`/`reminders`/`application_progress`/`requirement_checkoffs` via the anon/authenticated client — not automated; do this manually before a release if RLS policies change.
