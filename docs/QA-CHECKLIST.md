...
- **Follow-up** — the first real CI run (clean `npm ci`) exposed that "green today" only held for a stale local `node_modules`: `typescript@7.0.2` (the preview native-port compiler) is rejected by `typescript-eslint` (`<6.1.0`), and `eslint@10` breaks `eslint-config-next`'s bundled `eslint-plugin-react@7.37.5` (caps at `eslint ^9.7`). Fix: align the toolchain to the latest ecosystem-supported **stable** — `typescript@6.0.3`, `eslint@9.39.5` — add the missing `@types/jsdom` (jsdom ships no bundled types; the transitive dep was dropped on a clean resolve), align `react` to `19.2.8` to match `react-dom`, and regenerate `package-lock.json` in sync so `npm ci` passes. All four gates + `test:coverage` green on a clean install.
