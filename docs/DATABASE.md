...
### Source-discovery (FR22) — `source_index_pages`, `scholarship_candidates`

Migration `20260101000014_source_discovery.sql`. Where the source-watcher (above) only re-checks scholarships that already exist, this stage discovers **new** ones. Both tables follow the same convention: **RLS enabled, zero policies** (default-deny), service-role only, with the `grant ... to service_role` in the same migration.

- **`source_index_pages`** — the curator-managed registry of official index/listing pages the crawler reads (`/admin/source-pages`). `provider_id` (nullable FK), `index_url` (**`unique`**), `label`, `is_active`, `last_crawled_at`, `last_content_hash` (the change-gate: skip the LLM when the link set is unchanged), `last_error` (a persistently failing page surfaces to the curator), `created_at`. Partial index on `(last_crawled_at) where is_active`.
- **`scholarship_candidates`** — the discovery review queue. One row = one new scholarship the crawler drafted, awaiting review: `source_index_page_id`, `detail_url`, `content_hash`, `extracted` (jsonb — the full un-verified draft, reviewed/edited at promotion, never queried as live data), `citing_snippets` (jsonb — verbatim source evidence shown to the curator), `confidence` (`high`/`medium`/`low`, CHECK), `status` (`pending`/`approved`/`rejected`/`duplicate`, CHECK), `dedupe_key` (normalized `detail_url`), `promoted_scholarship_id`, `reviewed_by`/`reviewed_at`, `rejection_reason`. Indexes: `(created_at) where status='pending'` (queue), and a **partial unique dedupe index** on `(dedupe_key) where status='pending'`. A separate table (not `scholarship_suggestions`) because that queue is field-level and structurally forbids adding a whole scholarship row (`scholarship_suggestions_scholarships_update_only`), whereas a discovery **is** a whole new draft.

Both tables carry a trigger (`enforce_source_discovery_url_allowlist`) that runs `index_url`/`detail_url` through the same shared `is_allowlisted_url()` as the `scholarships` URL trigger — so an off-allowlist page can never be registered or filed, at the DB layer. Writes: the cron (`/api/cron/discover-sources`) writes candidates via the service-role client; curator promotion (`lib/actions/discoveries.ts`) routes the reviewed draft through the existing validated `upsertScholarship` action (creating a *draft* scholarship — never auto-published), and audit-logs. No anon/authenticated write path exists to either table.
