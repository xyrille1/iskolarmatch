    ...
    <section className="mt-10">
      ...
      <ul className="mt-3 flex flex-col gap-2 text-sm">
        {requirements.map((req) => (
          <li key={req.id} className="flex items-center justify-between border-b border-black/10 py-2">
            <span>
      ...
      <form action={addRequirementFormAction.bind(null, scholarshipId)} className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <input name="label" placeholder="Requirement label" required className="col-span-2 rounded border border-black/20 px-2 py-1.5" />
        <input name="sort_order" type="number" defaultValue={requirements.length} className="rounded border border-black/20 px-2 py-1.5" />
