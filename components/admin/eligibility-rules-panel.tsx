  ...
  return (
    <section className="mt-10">
      ...
      <ul className="mt-3 flex flex-col gap-2 text-sm">
        {rules.map((rule) => (
          <li key={rule.id} className="flex items-center justify-between border-b border-line py-2">
            <span>
              {rule.field} {rule.operator} {JSON.stringify(rule.value)}
              {rule.is_mandatory ? "" : " (bonus)"} -- &quot;{rule.human_label}&quot;
              {rule.guidance_text && (
                <span className="block text-muted">Guidance: {rule.guidance_text}</span>
              )}
            </span>
      ...
      <form action={addEligibilityRuleFormAction.bind(null, scholarshipId)} className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <select name="field" required className="rounded border border-line px-2 py-1.5">
        ...
        <select name="operator" required className="rounded border border-line px-2 py-1.5">
          {OPERATORS.map((o) => (
        ...
        <input
          ...
          required
          className="col-span-2 rounded border border-line px-2 py-1.5"
        />
        <input
          name="human_label"
