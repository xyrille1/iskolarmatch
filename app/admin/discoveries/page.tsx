...
export default async function DiscoveriesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      {candidates.length === 0 ? (
        <p className="mt-8 text-sm text-black/60">No pending discoveries.</p>
      ) : (
        <ul className="mt-8 flex flex-col gap-8">
          {candidates.map((c) => {
            ...
            return (
              <li key={c.id} className="rounded border border-black/10 p-4 text-sm">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-black/50">{c.sourceLabel ?? "unknown source"}</span>
                </div>
                ...
                <div className="mt-3 rounded bg-black/[0.03] p-3">
                  <p className="text-xs font-medium text-black/60">Detected — add on the edit page after promoting</p>
                  <div className="mt-1 text-xs text-black/70">
                    <span className="text-black/50">Deadline: </span>
                  ...
                  {(draft.eligibility_notes.length > 0 || draft.requirement_labels.length > 0) && (
                    <div className="mt-2 grid gap-3 sm:grid-cols-2">
                      {draft.eligibility_notes.length > 0 && (
                        <div>
                          <p className="text-xs text-black/50">Eligibility</p>
                          <ul className="mt-1 list-disc pl-5 text-xs text-black/70">
                ...
                {c.snippets.length > 0 && (
                  <details className="mt-3">
                    <summary className="cursor-pointer text-xs text-black/50">Source evidence ({c.snippets.length} sections)</summary>
                    <div className="mt-2 flex flex-col gap-2 text-xs text-black/60">
                      {c.snippets.map((s, i) => (
                        <div key={i}>
                ...
                <form action={promoteCandidateFormAction.bind(null, c.id)} className="mt-4 flex flex-col gap-2 border-t border-black/10 pt-4">
                  <p className="text-xs font-medium text-black/60">Review &amp; promote</p>
                  <div className="grid gap-2 sm:grid-cols-2">
                    <label className="flex flex-col gap-1">
                      <span className="text-xs text-black/50">Title</span>
                      <input name="title" required defaultValue={draft.title} className="rounded border border-black/20 px-2 py-1.5" />
                    </label>
                    <label className="flex flex-col gap-1">
                    ...
                    <label className="flex flex-col gap-1">
                      <span className="text-xs text-black/50">Provider</span>
                      <select name="provider_id" required defaultValue={matchedProvider?.id ?? ""} className="rounded border border-black/20 px-2 py-1.5">
                    ...
                    <label className="flex flex-col gap-1">
                      <span className="text-xs text-black/50">Coverage type</span>
                      <select name="coverage_type" defaultValue={draft.coverage_type} className="rounded border border-black/20 px-2 py-1.5">
                  ...
                  <label className="flex flex-col gap-1">
                    <span className="text-xs text-black/50">Summary</span>
                    <textarea name="summary" rows={2} defaultValue={draft.summary ?? ""} className="rounded border border-black/20 px-2 py-1.5" />
                  </label>
                  <label className="flex flex-col gap-1">
                    <span className="text-xs text-black/50">Benefit summary</span>
                    <input name="benefit_summary" defaultValue={draft.benefit_summary ?? ""} className="rounded border border-black/20 px-2 py-1.5" />
                  </label>
                  <label className="flex flex-col gap-1">
                    <span className="text-xs text-black/50">Official URL (allowlist-enforced)</span>
                    <input name="official_url" required defaultValue={draft.official_url} className="rounded border border-black/20 px-2 py-1.5" />
                  </label>
                  <label className="flex flex-col gap-1">
                    <span className="text-xs text-black/50">Application URL (optional, allowlist-enforced)</span>
                    <input name="application_url" defaultValue={prefillApplicationUrl} placeholder="Leave blank if off-allowlist" className="rounded border border-black/20 px-2 py-1.5" />
                  </label>
                ...
                <form action={rejectCandidateFormAction.bind(null, c.id)} className="mt-3 flex items-center gap-2">
                  <input type="text" name="reason" placeholder="Reason (optional)" className="rounded border border-black/20 px-2 py-1 text-xs" />
